...
-(id)initWithViewController:(ViewController *)vc {
    self = [super initWithRealm:@"CustomAuthenticatorRealm"];
    self.vc = vc;
    return self;
}
...
-(BOOL) isCustomResponse:(WLResponse *)response {
    if(response && response.responseJSON){
        if ([response.responseJSON objectForKey:@"authStatus"]) {
            NSString* authRequired = (NSString*) [response.responseJSON objectForKey:@"authStatus"];
